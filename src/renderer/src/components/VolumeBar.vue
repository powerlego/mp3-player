...
function getCurrentVolume(event: MouseEvent | TouchEvent) {
  ...
  if (!volumeBar.value) {
    return { currentVolumeValue: volume.value, currentVolumePositionValue: `${((lastVolume.value / 1) * 100).toFixed(2)}%` };
  }
...
<template>
  <div v-if="volume === -1" ref="volumeBar" class="flex flex-row items-center justify-center flex-1 w-full h-5"
    role="progressbar">
  ...
  <div v-else-if="!isAudioAvailable" ref="volumeBar" class="flex flex-row items-center justify-center flex-1 w-full h-5"
    role="progressbar">
    <div class="box-border relative w-full h-1 rounded-full bg-gray-350 dark:bg-gray-750">
      <div class="box-border absolute z-10 h-full bg-gray-300 rounded-full dark:bg-gray-600" />
    </div>
  </div>
  <div v-else ref="volumeBar" class="flex flex-row items-center justify-center flex-1 w-full h-5 group"
    role="progressbar" aria-valuemax="100" aria-valuemin="0"
    :aria-valuenow="Number(currentVolumePosition.split('%')[0])" @mousedown="handleMouseDownOrTouchStart"
    @touchstart="handleMouseDownOrTouchStart" @contextmenu="($event) => $event.preventDefault()">
    <div className="relative box-border h-1 w-full rounded-full bg-gray-350 dark:bg-gray-750">
      <div class="box-border absolute z-20 rounded-full h-13/4 aspect-square group-hover:scale-100"
        :class="{ 'scale-100': isDragging, 'scale-0': !isDragging }" :style="{ 'left': currentVolumePosition }">
        <div
      ...
      <div class="box-border absolute z-10 h-full rounded-full group-hover:bg-green-700 dark:group-hover:bg-green-600"
