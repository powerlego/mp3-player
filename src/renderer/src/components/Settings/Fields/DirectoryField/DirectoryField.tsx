...
export default class DirectoryField extends React.Component<DirectoryFieldProps> {
        ...
        <Tooltip className="w-full" content={this.description}>
          <div onClick={this.choose.bind(this)}>
            <div className="w-full px-2 py-1 font-bold border-x-2 border-t-2 border-solid bg-gray-150 dark:bg-gray-750 border-gray-350 dark:border-gray-700 rounded-t-lg">
              {this.multiSelections ? "Folders" : "Folder"}:&nbsp;
            </div>
            {this.value
              ? (
          ...
          <button
            aria-label={btnLabel}
            className="bg-gray-50 dark:bg-gray-800 px-2 rounded-md py-1 enabled:hover:bg-gray-100 dark:enabled:hover:bg-gray-700 transition ease-in-out disabled:bg-gray-100 dark:disabled:bg-gray-850 disabled:cursor-not-allowed enabled:active:bg-gray-150 dark:enabled:active:bg-gray-650"
  ...
  choose() {
    ...
    const properties: OpenDialogSyncOptions["properties"] = ["openDirectory", "createDirectory"];
